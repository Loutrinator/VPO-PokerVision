...
cv::Mat Image::convertToHsv() {
	...
	DataQualifier::brightnessContrast(dst,1,1.6);
	cv::Mat hsv;
	cv::cvtColor(dst, hsv, cv::COLOR_BGR2HSV);

	DataQualifier::showImage(dst, "mat", 720);
	...
	DataQualifier::showImage(shiftedH, "shiftedH", 720);
	cv::Mat cannyH;
	cv::Canny(H, cannyH, 200, 100);
	DataQualifier::showImage(cannyH, "cannyH", 720);
	//cv::Mat cannyS;
	//cv::Canny(S, cannyS, 200, 100);
	//DataQualifier::showImage(cannyS, "cannyS", 720);
