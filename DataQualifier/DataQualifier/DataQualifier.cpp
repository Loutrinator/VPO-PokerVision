...
DataQualifier::DataQualifier(bool pip, int nbMatch, float angleMargin): pipOnly(pip), nbPointsToMatch(nbMatch)
{
...
void DataQualifier::setCardsDataset(const cv::Mat& cardsFile, int width, int height)
{
...
void DataQualifier::findCards(Image& img, bool enableLogs)
{
	int cardsFound = 0;
	for (Card card : cards) {
		bfm = cv::BFMatcher::create();
...
void DataQualifier::removeOverlapingImages(float minDist)
{
...
void DataQualifier::groupCards(float minDist)
{
	std::vector<std::vector<int>> groups = getGroupCardIdsByDistance(minDist);
...
cv::Point2f DataQualifier::getBarrycenter(std::vector<cv::Point2f> points)
{
	float x = 0;
	float y = 0;
	int count = 0;
...
void DataQualifier::divide(cv::Mat& img,cv::Vec3b rgb)
{
	cv::Mat whitened;
...
void DataQualifier::brightnessContrast(cv::Mat& img, double contrast, int brightness)
{
...
void DataQualifier::showImage(cv::Mat& img, std::string name, int width)
{
...
void DataQualifier::showResult(const Image& img, bool showProcessedImage, bool showCards, bool showPoints, bool showText, bool showROI, bool showBarrycenters)
{
...
bool DataQualifier::angleIsValid(float angle)
{
	return (angle > minAngle && angle < maxAngle);
}

cv::Scalar DataQualifier::HSV2BGR(float h, float s, float v)
{
...
cv::Scalar DataQualifier::BGR2HSV(float b, float g, float r)
{
...
std::vector<std::vector<int>> DataQualifier::getGroupCardIdsByDistance(float dist)
{
